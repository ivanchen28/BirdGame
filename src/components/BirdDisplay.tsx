...
export function BirdDisplay({ bird, cardHeight = CARD_HEIGHT }: BirdDisplayProps) {
  ...
  const eggs = useMemo(() => Array.from({ length: bird["Egg limit"] }, (_, i) => i), [bird]);
  ...
  // Resolve nest type to icon name: "star" → "star", "none" → null, else lowercase
  const nestType = useMemo(() => {
    const nestMap: Record<string, string | null> = { none: null };
